    ...
    public class UserAuthenticationController : Controller
	{
		...
		public async Task<IActionResult> Login(LoginVM model)
		{
			...
			if (loginedUser !=null)
			{
				...
				if (result.StatusString == "1")
				{
					...
					{
						return StatusCode(StatusCodes.Status200OK, new Status() { StatusString = "Başarısız", Message = "EmailConfirmed is Not found!" });
					}
				}
				else
				{
		...
		public async Task<IActionResult> SendMail(string eMail)
		{
			...
			if (loginedUser != null)
			{
				...
				,{"ŞİFRE YENİLEME",loginedUser.Email.ToString() } }, "Şifre Yenilemesi Hk.", $"<h1>Merhaba</h1><p>Şifrenizi yenilemek için {resetPassword} şifreyi eski şifre kısmına yazınız. </p>");
					...
					await _userService.SaveDataBaseAsync();
					return StatusCode(StatusCodes.Status200OK, new Status() { StatusString = "Başarılı", Message = "Mail sent successfully!" });
				}
			...
			{
				return StatusCode(StatusCodes.Status404NotFound, new Status() { StatusString = "Başarısız", Message = "Mail is Not Found!" });
			}
		}
		...
		public async Task<IActionResult> ConfirmEmail(string token, string email)
		{
			...
			if (user != null)
			{
				...
				if (result.Succeeded)
				{
					return StatusCode(StatusCodes.Status200OK, new Status() { StatusString = "Onaylama başarılı!", Message = "User's mail has been confirmed!" });
				}
				else
				{
					return StatusCode(StatusCodes.Status400BadRequest, new Status() { StatusString = "Onaylama başarısız!", Message = "User's token information is incorrect!" });
				}
			}
			else
			{
				return StatusCode(StatusCodes.Status404NotFound, new Status() { StatusString = "Kullanıcı sistemde bulunmamaktadır!", Message = "User is not found!" });
			}
		}
